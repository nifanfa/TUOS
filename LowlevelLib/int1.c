...
void init_int() {
	...
	IDTDescriptor* desc = &idt_descriptor;
	...
	//We can't use struct because of MSVC's shitty alignment makes it go wrong
	*(uint16_t*)((uintptr_t)desc) = (uint16_t)((sizeof(IDTEntry) * 256) - 1);
	*(uint64_t*)((uintptr_t)desc + 2) = (uint64_t)idt;
	__lidt(desc);
}