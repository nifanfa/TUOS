...
namespace ConsoleApp1
{
    ...
    internal unsafe class NETv4
    {
        ...
        public static bool DHCPDiscover()
        {
            ...
            Console.Write($"[DHCP] configured. IP:{Convert.ToString(IP.P1, 10)}.{Convert.ToString(IP.P2, 10)}.{Convert.ToString(IP.P3, 10)}.{Convert.ToString(IP.P4, 10)}\n");
        ...
        public static int MakeDHCPRequestPacket(byte* buffer,uint xid,byte type) 
        {
            ...
            Allocator.ZeroFill(dhcp_hdr, (ulong)sizeof(DHCPHeader));
        ...
        public static int MakeUDPPacket(byte* buffer, UDPClient client, byte* data, int length)
        {
            ...
            Allocator.MemCpy(p, data, (ulong)length);
            p += length;
        ...
        public static void ICMPOnData(IPv4Header* ip_hdr, byte* buffer)
        {
            ...
            if (icmp_hdr->Type == 8)
            {
                ...
                p += sizeof(IPv4Header);
                Allocator.MemCpy(p, buffer, (ulong)length);
                if (Sender != null)
                {
